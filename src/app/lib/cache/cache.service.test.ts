...
class StubDriver implements ICacheDriver {
  ...
  getOrSet<A>(
    ...
    return factory().pipe(
      Effect.tap((value: NoInfer<A>): Effect.Effect<void, CacheError> =>
        Effect.sync(() => {
  ...
  get<A>(key: string): Effect.Effect<Option.Option<A>, CacheError> {
    if (this.store.has(key)) {
      return Effect.succeed(Option.some(this.store.get(key) as A));
    }
...
describe("CacheService.", () => {
   ...
   * and returns the cached value on subsequent calls — the core
   * stampede-protection contract.
   */
  it("Calls factory once and serves the cached value on subsequent hits.", async () => {
    let calls = 0;
    const factory: () => Effect.Effect<number, CacheError> = (): Effect.Effect<
      number,
      CacheError
    > =>
      Effect.sync((): number => {
    ...
    const first: number = await Effect.runPromise(
      service.getOrSet("k", 60, factory),
    );
    const second: number = await Effect.runPromise(
      service.getOrSet("k", 60, factory),
    );
  ...
  it("Returns Option.none() after a key is deleted.", async () => {
    ...
    const result: Option.Option<string> = await Effect.runPromise(
      service.get<string>("x"),
    );
    expect(Option.isNone(result)).toBe(true);
  });

  /**
   * Verifies that `get` returns `Option.some` for a key that exists —
   * confirming the Option wrapping is applied correctly by the service layer.
   */
  it("Returns Option.some with the stored value for an existing key.", async () => {
    await Effect.runPromise(service.set("y", 99, 60));

    const result: Option.Option<number> = await Effect.runPromise(
      service.get<number>("y"),
    );
   ...
   * status field — required by error-boundary middleware that inspects
   * `.status`.
   */
  it("CacheError carries the expected tag and status.", () => {
    const error: CacheError = new CacheError({
      message: "test",
      status: HttpStatus.internal,
    });
    expect(error._tag).toBe("CacheError");
    expect(error.status).toBe(HttpStatus.internal);
  });
});
